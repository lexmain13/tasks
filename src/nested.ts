...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    const totalPubPoints = pubQuestions.reduce(
        (currentSum: number, question: Question) =>
            currentSum + question.points,
        0
    );
...
export function toCSV(questions: Question[]): string {
    const questionsHeader = "id,name,options,points,published\n";
    const questionsCSV = questions
        ...
        .join("\n");
    return questionsHeader + questionsCSV;
}
...
export function publishAll(questions: Question[]): Question[] {
    const pubAllQuestions = questions.map(
        (question: Question): Question => ({ ...question, published: true })
    );
    return pubAllQuestions;
}
...
export function sameType(questions: Question[]): boolean {
    const shortAnswerEqual = questions.every(
        (question: Question): boolean =>
            question.type === "short_answer_question"
    );
    const multipleAnswerEqual = questions.every(
        (question: Question): boolean =>
            question.type === "multiple_choice_question"
    );
    if (shortAnswerEqual) {
        return true;
    } else if (multipleAnswerEqual) {
        return true;
    } else {
        return false;
    }
...
export function addNewQuestion(
    ...
    const updatedQuestionArray = [...questions, blankQuestion];
